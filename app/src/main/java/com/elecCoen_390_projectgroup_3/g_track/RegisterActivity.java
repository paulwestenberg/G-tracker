...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    EditText surName,name,userName,eMail,password,confirmPassword;
    Button saveRegisterBtn,cancelRegisterBtn;
    ProgressBar progressBarRegister;
    String surNameString,nameString,userNameString,eMailString,passwordString,confirmPasswordString;
    ...
    private void registerUserMethod() {
        ...
        if(userNameString.isEmpty()){userName.setError("Fill the User Name Please!");userName.requestFocus(); return;}
        ...
        if(passwordString.length()<6){password.setError("The password should be more then 6 charrecters");password.requestFocus(); return;}
        ...
        mAuth.createUserWithEmailAndPassword(eMailString,passwordString).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()){User user = new User(surNameString,nameString,userNameString,eMailString);
